...
app.get('/comanda', (req, res) => {
  const productes = db.prepare("SELECT Productes.id ,productes.nom AS nombre_producto FROM PRODUCTES").all();
  const empreses = db.prepare("SELECT empreses.id , Empreses.nom AS nombre_empresa FROM EMPRESES").all();
  res.render("comanda", { empreses: empreses , productes: productes })
})

app.post('/comanda', (req, res) => {
  if (req.body.empresa_id && req.body.producte_id) {
    const statement = db.prepare("INSERT INTO comandes (empresa_id,producte_id) VALUES (?,?)")
    const info = statement.run(req.body.empresa_id , req.body.producte_id)
  }
